...
FStatus FConfigService::LoadKnownConfigs(FLogger* logger)
{
    ...
    {
        FStatus status = LoadConfig(std::string(name));

        if (logger && status.IsOk())
        {
            logger->Info(std::string("loaded cfg: ") + std::string(name));
        }
    }
...
std::optional<std::string> FConfigService::FindString(std::string_view key) const
{
    ...
    {
        if (auto value = pair.second.FindString(key))
        {
            return value;
        }
    }
...
std::optional<int64> FConfigService::FindInt(std::string_view key) const
{
    for (const auto& pair : Documents)
    {
        if (auto value = pair.second.FindInt(key))
        {
            return value;
        }
    }
