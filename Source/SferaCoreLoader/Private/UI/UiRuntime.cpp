...
std::string WideToUtf8(const std::wstring& text) {
    ...
    if (required > 0) { WideCharToMultiByte(CP_UTF8, 0, text.c_str(), static_cast<int>(text.size()), utf8.data(), required, nullptr, nullptr); }
    return utf8;
}
...
bool Utf8EqualsWideNoCase(const std::string& utf8, const std::wstring& wide) {
    ...
    auto lowerWide = [](std::wstring value) { std::transform(value.begin(), value.end(), value.begin(), [](wchar_t ch) { return static_cast<wchar_t>(std::towlower(ch)); }); return value; };
        ...
        if (inControl) {
            ...
            if (controlNestedDepth > 0) {
                if (controlNestedDepth == 1 && !activeControlSection.empty()) {
                    ...
                    else if (key == "uncheckedimage") { button.UncheckedImage = ParseQuoted(line); }
                }
                continue;
            }
            std::istringstream input(line);
            ...
            else if (key == "maxsymbols") { input >> control.MaxSymbols; }
            else if (key == "enteredonfocus") { control.EnteredOnFocus = ParseBool(input, control.EnteredOnFocus); }
            else if (key == "hotkey") { control.HotKey = ParseQuoted(line); }
            else if (key == "hypertext") { control.HyperTextResource = ParseQuoted(line); }
            else if (key == "imageoffset") { input >> control.ImageOffset.X >> control.ImageOffset.Y; }
            else if (key == "group") { input >> control.Group; }
            ...
            else if (key == "range") { input >> control.RangeMin >> control.RangeMax; }
            else if (key == "progresspos") { input >> control.ProgressPos; }
            else if (key == "statusshow") { input >> control.StatusShow; }
            else if (key == "statuspos") { input >> control.StatusPos.X >> control.StatusPos.Y; }
...
FStatus FUiRuntime::Initialize(const FResourceManager& resources, const FUiBootstrapDesc& desc, FLogger* logger) {
    ...
    if (!pickPerson.IsOk() && logger) { logger->Warning("character-select UI is not available: " + pickPerson.Status().Message()); }
    auto createPerson = LoadUiWindowFromResource(resources, Bootstrap.CreatePersonWindowResource);
    if (!createPerson.IsOk() && logger) { logger->Warning("create-character UI is not available: " + createPerson.Status().Message()); }
    auto deleteCharacter = LoadUiWindowFromResource(resources, Bootstrap.DeleteCharacterWindowResource);
    if (!deleteCharacter.IsOk() && logger) { logger->Warning("delete-character UI is not available: " + deleteCharacter.Status().Message()); }
    auto connectMessage = LoadUiWindowFromResource(resources, Bootstrap.ConnectMessageWindowResource);
    if (!connectMessage.IsOk() && logger) { logger->Warning("confirmation UI is not available: " + connectMessage.Status().Message()); }
    auto message = LoadUiWindowFromResource(resources, Bootstrap.MessageWindowResource);
    if (!message.IsOk() && logger) { logger->Warning("generic message UI is not available: " + message.Status().Message()); }
    ...
    if (pickPerson.IsOk()) { PickPerson = std::move(pickPerson.Value()); }
    if (createPerson.IsOk()) { CreatePerson = std::move(createPerson.Value()); }
    if (deleteCharacter.IsOk()) { DeleteCharacter = std::move(deleteCharacter.Value()); }
    if (connectMessage.IsOk()) { ConnectMessage = std::move(connectMessage.Value()); }
    if (message.IsOk()) { Message = std::move(message.Value()); }
    Ready = true;
    ...
    if (!PickPerson.Name.empty()) { AddStatusLine("ui: character window loaded, controls=" + std::to_string(PickPerson.Controls.size()) + ", sprites=" + std::to_string(PickPerson.Sprites.size())); }
    if (!CreatePerson.Name.empty()) { AddStatusLine("ui: create window loaded, controls=" + std::to_string(CreatePerson.Controls.size())); }
    if (!DeleteCharacter.Name.empty()) { AddStatusLine("ui: delete window loaded, controls=" + std::to_string(DeleteCharacter.Controls.size())); }
    if (!ConnectMessage.Name.empty()) { AddStatusLine("ui: confirm window loaded, controls=" + std::to_string(ConnectMessage.Controls.size())); }
    if (!Message.Name.empty()) { AddStatusLine("ui: message window loaded, controls=" + std::to_string(Message.Controls.size())); }
    if (logger) { logger->Info("UI runtime initialized: strings=" + std::to_string(StringTable.size()) + ", connection=" + Connection.Name + ", controls=" + std::to_string(Connection.Controls.size()) + ", pick_person=" + PickPerson.Name + ", create_person=" + CreatePerson.Name + ", connect_message=" + ConnectMessage.Name + ", message=" + Message.Name); }
    return FStatus::Ok();
}
...
FUiRectF FUiRuntime::BuildWindowRect(const FUiWindowDef& window, const tagRECT& clientRect) const {
    ...
    if (window.AlignRightX) { x = clientW - w + x; }
    else if (window.AlignCenterX) { x = (clientW - w) * 0.5f + x; }
    if (window.AlignRightY) { y = clientH - h + y; }
    else if (window.AlignCenterY) { y = (clientH - h) * 0.5f + y; }
    return FUiRectF{std::floor(x), std::floor(y), w, h};
}
...
const FUiControlDef* FUiRuntime::HitTestCharacterSelect(int32 x, int32 y, const tagRECT& clientRect) const {
    ...
    for (auto it = PickPerson.Controls.rbegin(); it != PickPerson.Controls.rend(); ++it) {
        const FUiControlDef& control = *it;
        if (!ControlCanReceiveMouse(control)) { continue; }
        FUiRectF r{wr.X + static_cast<float>(control.Rect.X), wr.Y + static_cast<float>(control.Rect.Y), static_cast<float>(std::max(1, control.Rect.W)), static_cast<float>(std::max(1, control.Rect.H))};
        if (EqualsNoCase(control.ClassId, "SPINBUTTON") && (control.Rect.W <= 0 || control.Rect.H <= 0)) { r.W = 37.0f; r.H = 26.0f; }
        if (static_cast<float>(x) >= r.X && static_cast<float>(y) >= r.Y && static_cast<float>(x) < r.X + r.W && static_cast<float>(y) < r.Y + r.H) { return &control; }
    }
    return nullptr;
}
...
void FUiRuntime::ActivateCharacterControl(const FUiControlDef& control, FLogger* logger) {
    Actions.LastControlId = control.Id;
    if (control.Id >= 63 && control.Id <= 65) { SelectedSlot = std::clamp(control.Id - 63, 0, 2); ActiveCharacterEditId = 60 + SelectedSlot; SceneCameraFocusId = 0; SyncCharacterSelectControls(); Actions.LastAction = "character_slot_" + std::to_string(SelectedSlot); }
    else if (control.Id >= 60 && control.Id <= 62) { SelectedSlot = std::clamp(control.Id - 60, 0, 2); ActiveCharacterEditId = control.Id; SceneCameraFocusId = 0; SyncCharacterSelectControls(); Actions.LastAction = "character_edit_" + std::to_string(SelectedSlot); }
    else if (control.Id >= 12 && control.Id <= 16) {
        const int32 delta = CharacterSpinDelta < 0 ? -1 : 1;
        auto cycle = [delta](int32 value, int32 count) { if (count <= 0) { return 0; } value = (value + delta) % count; return value < 0 ? value + count : value; };
        if (control.Id == 12) { Appearance.Gender = cycle(Appearance.Gender, 2); Appearance.Face = 0; Appearance.Hair = 0; Appearance.HairColor = 0; Appearance.Tattoo = 0; }
        else if (control.Id == 13) { Appearance.Face = cycle(Appearance.Face, CharacterAppearanceOptionCount(13)); }
        else if (control.Id == 14) { Appearance.Hair = cycle(Appearance.Hair, CharacterAppearanceOptionCount(14)); }
        else if (control.Id == 15) { Appearance.HairColor = cycle(Appearance.HairColor, CharacterAppearanceOptionCount(15)); }
        else if (control.Id == 16) { Appearance.Tattoo = cycle(Appearance.Tattoo, CharacterAppearanceOptionCount(16)); }
        ClampCharacterAppearance();
        SyncCharacterSelectControls();
        Actions.LastAction = "character_appearance_changed";
    }
    else if (control.Id == CharacterContinueButtonId) { if (SelectedCharacterPresent()) { Actions.LastAction = "character_enter_requested"; } else { ShowCreateConfirmation(); } }
    else if (control.Id == CharacterExitButtonId || control.SendQuit) { ShowExitConfirmation(); }
    else if (control.Id == CharacterDeleteButtonId) { ShowDeleteConfirmation(); }
    else { Actions.LastAction = "character_click_" + std::to_string(control.Id); }
    if (logger) { logger->Info("UI action: " + Actions.LastAction); }
}
...
void FUiRuntime::ActivateModalControl(const FUiControlDef& control, FLogger* logger) {
    ...
    if (current == EUiModalDialog::CharacterCreate) { if (control.Id == 1) { DismissModal(); Actions.LastAction = "character_create_confirmed"; } else if (control.Id == 2 || control.SendQuit) { DismissModal(); Actions.LastAction = "character_create_cancelled"; } else { Actions.LastAction.clear(); } }
    else if (current == EUiModalDialog::CharacterDelete) { if (control.Id == 5 && ModalEditMatchesSelectedCharacter()) { DismissModal(); Actions.LastAction = "character_delete_confirmed"; } else if (control.Id == 1 || control.SendQuit) { DismissModal(); Actions.LastAction = "character_delete_cancelled"; } else { Actions.LastAction = "character_delete_name_required"; } }
    else if (current == EUiModalDialog::CharacterExit) { if (control.Id == 1) { DismissModal(); Actions.LastAction = "character_back_confirmed"; } else if (control.Id == 2 || control.SendQuit) { DismissModal(); Actions.LastAction = "character_back_cancelled"; } else { Actions.LastAction.clear(); } }
    else { DismissModal(); Actions.LastAction = "modal_closed"; }
...
bool FUiRuntime::HandleInputFrame(const FInputSnapshot& input, const tagRECT& clientRect, FLogger* logger) {
    ...
    if (Modal != EUiModalDialog::None) { hovered = HitTestModal(input.MouseX, input.MouseY, clientRect); }
    else { hovered = CurrentMode == EUiRuntimeMode::CharacterSelect ? HitTestCharacterSelect(input.MouseX, input.MouseY, clientRect) : HitTestConnection(input.MouseX, input.MouseY, clientRect); }
    const int32 newHover = hovered ? hovered->Id : 0;
    int32 spinHoverDirection = 0;
    if (hovered && CurrentMode == EUiRuntimeMode::CharacterSelect && Modal == EUiModalDialog::None && EqualsNoCase(hovered->ClassId, "SPINBUTTON")) { spinHoverDirection = CharacterSpinDeltaForPoint(*hovered, input.MouseX, input.MouseY, clientRect); }
    if (newHover != Actions.HoverControlId || spinHoverDirection != Actions.SpinHoverDirection) { Actions.HoverControlId = newHover; Actions.SpinHoverDirection = spinHoverDirection; changed = true; }
    ...
    if (input.LeftReleased) {
        ...
        SceneRotateDragActive = false;
        changed = true;
        if (hovered && hovered->Id != 0 && hovered->Id == pressed) { if (Modal != EUiModalDialog::None) { ActivateModalControl(*hovered, logger); } else if (CurrentMode == EUiRuntimeMode::CharacterSelect) { ActivateCharacterControl(*hovered, logger); } else { ActivateControl(*hovered, logger); } }
    }
    if (Modal != EUiModalDialog::None) {
        if (Modal == EUiModalDialog::CharacterDelete && Actions.FocusedControlId == 4) {
            if (input.BackspacePressed && !ModalEditText.empty()) { RemoveLastUtf8Codepoint(ModalEditText); changed = true; }
            if (!input.TypedText.empty()) { AppendUtf8Limited(ModalEditText, input.TypedText, 20); changed = true; }
        }
        if (input.EnterPressed) {
            ...
            else { DismissModal(); Actions.LastAction = current == EUiModalDialog::CharacterCreate ? "character_create_confirmed" : current == EUiModalDialog::CharacterDelete ? "character_delete_confirmed" : "character_back_confirmed"; }
            changed = true;
        }
        return changed;
    }
    if (CurrentMode == EUiRuntimeMode::Login && (Actions.FocusedControlId == LoginEditId || Actions.FocusedControlId == PasswordEditId)) {
        ...
        if (!input.TypedText.empty()) { const size_t limit = Actions.FocusedControlId == PasswordEditId ? MaxPasswordChars : MaxLoginChars; AppendUtf8Limited(target, input.TypedText, limit); changed = true; }
    ...
    if (CurrentMode == EUiRuntimeMode::CharacterSelect && ActiveCharacterEditId >= 60 && ActiveCharacterEditId <= 62 && !CharacterActionLocked) {
        ...
        for (wchar_t ch : typed) { if (target.size() < MaxCharacterNameChars && IsCharacterNameChar(ch)) { target.push_back(ch); changed = true; } }
    }
    if (CurrentMode == EUiRuntimeMode::Game) {
        if (input.BackspacePressed && !GameChat.empty()) { RemoveLastUtf8Codepoint(GameChat); changed = true; }
        if (!input.TypedText.empty()) { GameChat += input.TypedText; GameChat = TrimUtf8ToCodepoints(GameChat, 240); changed = true; }
    }
    if (input.EnterPressed) { if (CurrentMode == EUiRuntimeMode::CharacterSelect && !SelectedCharacterPresent() && SelectedCharacterCanCreate()) { ShowCreateConfirmation(); } else if (CurrentMode == EUiRuntimeMode::Game) { Actions.LastAction = "game_chat_enter"; } else { Actions.LastAction = CurrentMode == EUiRuntimeMode::CharacterSelect ? "character_enter_requested" : "login_requested"; } changed = true; if (logger) { logger->Info("UI action: " + Actions.LastAction); } }
    return changed;
}
...
void FUiRuntime::SetMode(EUiRuntimeMode mode) {
    ...
    Actions.SpinPressedDirection = 0;
    if (CurrentMode == EUiRuntimeMode::CharacterSelect) { SceneCameraFocusId = 0; SyncCharacterSelectControls(); }
}

std::wstring FUiRuntime::DefaultCharacterNameForSlot(int32 slot) const {
    std::wstring name;
    for (wchar_t ch : Utf8ToWideLocal(Actions.LoginText)) { if (IsCharacterNameChar(ch)) { name.push_back(ch); } if (name.size() >= 12) { break; } }
    if (name.empty()) { name = L"Hero"; }
    if (!name.empty() && name.front() >= L'0' && name.front() <= L'9') { name.insert(name.begin(), L'C'); }
    if (slot > 0 && name.size() < 12) { name.push_back(static_cast<wchar_t>(L'1' + slot)); }
...
void FUiRuntime::SetCharacterSlots(const std::array<FCharacterSlotInfo, 3>& slots) {
    ...
    bool found = false;
    for (int32 i = 0; i < 3; ++i) { if (CharacterSlotState[static_cast<size_t>(i)].Present) { firstSelectable = i; found = true; break; } }
    if (!found) { for (int32 i = 0; i < 3; ++i) { if (CharacterSlotState[static_cast<size_t>(i)].CanCreate) { firstSelectable = i; found = true; break; } } }
    SelectedSlot = std::clamp(found ? firstSelectable : SelectedSlot, 0, 2);
    for (int32 i = 0; i < 3; ++i) { const auto& slot = CharacterSlotState[static_cast<size_t>(i)]; CharacterNameEdits[static_cast<size_t>(i)] = slot.Present ? slot.Name : DefaultCharacterNameForSlot(i); }
...
FUiControlDef* FUiRuntime::MutableCharacterControlById(int32 id) {
    for (auto& control : PickPerson.Controls) { if (control.Id == id) { return &control; } }
    return nullptr;
}

const FUiControlDef* FUiRuntime::CharacterControlById(int32 id) const {
    for (const auto& control : PickPerson.Controls) { if (control.Id == id) { return &control; } }
    return nullptr;
}
...
void FUiRuntime::SyncCharacterSelectControls() {
    ...
    for (int32 i = 0; i < 3; ++i) {
        ...
        if (auto* edit = MutableCharacterControlById(60 + i)) { edit->Hidden = i != selected || slot.Present || !slot.CanCreate; edit->Disabled = edit->Hidden || CharacterActionLocked; }
        if (auto* radio = MutableCharacterControlById(63 + i)) { radio->Hidden = false; radio->Disabled = CharacterActionLocked || (!slot.Present && !slot.CanCreate); }
    }
    ...
    for (int32 id = 12; id <= 16; ++id) { if (auto* control = MutableCharacterControlById(id)) { control->Hidden = false; control->Disabled = !canEditAppearance; } }
    if (auto* del = MutableCharacterControlById(CharacterDeleteButtonId)) { del->Disabled = CharacterActionLocked || !selectedSlot.Present; }
    if (auto* exit = MutableCharacterControlById(CharacterExitButtonId)) { exit->Disabled = CharacterActionLocked; }
    if (auto* cont = MutableCharacterControlById(CharacterContinueButtonId)) { cont->Disabled = CharacterActionLocked || (!selectedSlot.Present && !selectedSlot.CanCreate); }
    if (!selectedSlot.Present && selectedSlot.CanCreate && !CharacterActionLocked) { ActiveCharacterEditId = 60 + selected; }
}
...
FCharacterCreationAppearance FUiRuntime::SelectedCharacterSceneAppearance() const {
    ...
    if (info.Present) { out.Female = info.Female; out.Face = ClampIndexToCount(info.Face, info.Female ? AppearanceRules.FemaleFaceCount : AppearanceRules.MaleFaceCount); out.Hair = ClampIndexToCount(info.Hair, info.Female ? AppearanceRules.FemaleHairCount : AppearanceRules.MaleHairCount); out.HairColor = ClampIndexToCount(info.HairColor, AppearanceRules.HairColorCount); out.Tattoo = ClampIndexToCount(info.Tattoo, AppearanceRules.TattooCount); }
    else { out.Female = Appearance.Gender != 0; out.Face = ClampIndexToCount(Appearance.Face, out.Female ? AppearanceRules.FemaleFaceCount : AppearanceRules.MaleFaceCount); out.Hair = ClampIndexToCount(Appearance.Hair, out.Female ? AppearanceRules.FemaleHairCount : AppearanceRules.MaleHairCount); out.HairColor = ClampIndexToCount(Appearance.HairColor, AppearanceRules.HairColorCount); out.Tattoo = ClampIndexToCount(Appearance.Tattoo, AppearanceRules.TattooCount); }
    return out;
}
...
float FUiRuntime::CharacterProgressRatio(int32 controlId) const {
    const auto& slot = CharacterSlotState[static_cast<size_t>(std::clamp(SelectedSlot, 0, 2))];
    const bool occupied = slot.Present;
    auto ratio = [](int32 current, int32 maximum) { return maximum <= 0 ? 0.0f : std::clamp(static_cast<float>(current) / static_cast<float>(maximum), 0.0f, 1.0f); };
...
std::string FUiRuntime::CharacterTitleText() const {
    ...
    switch (slot.TitleId) { case 14: name = "странник"; break; default: break; }
...
std::string FUiRuntime::CharacterDegreeText() const {
    ...
    if (!slot.Present) { return {}; }
    const char* name = nullptr;
    switch (slot.DegreeId) { case 112: name = "неучёный"; break; default: break; }
...
std::string FUiRuntime::CharacterKarmaText() const {
    ...
    if (Bootstrap.Lang == 1) {
        switch (karma) { case 1: return "Karma: very bad"; case 2: return "Karma: bad"; case 4: return "Karma: good"; case 5: return "Karma: benign"; default: return "Karma: neutral"; }
    }
    switch (karma) { case 1: return "Карма: очень плохая"; case 2: return "Карма: плохая"; case 4: return "Карма: хорошая"; case 5: return "Карма: добрая"; default: return "Карма: нейтральная"; }
}

std::string FUiRuntime::CharacterProgressText(const FUiControlDef& control) const {
    const auto& slot = CharacterSlotState[static_cast<size_t>(std::clamp(SelectedSlot, 0, 2))];
    if (!slot.Present) { return {}; }
    auto pairText = [](int32 current, int32 maximum) { return std::to_string(std::max(0, current)) + " / " + std::to_string(std::max(0, maximum)); };
...
std::string FUiRuntime::CharacterControlText(const FUiControlDef& control) const {
    ...
    if (control.Id >= 60 && control.Id <= 62) {
        ...
        if (ActiveCharacterEditId == control.Id && !CharacterActionLocked) { text.push_back(L'_'); }
