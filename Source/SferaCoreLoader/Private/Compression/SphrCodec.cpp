...
FSphrProbe FSphrCodec::Probe(const FByteArray& bytes) {
    ...
    uint8 decoded[4] = {0,0,0,0};
    for (size_t i = 0; i < 4; ++i) { decoded[i] = static_cast<uint8>(bytes[4 + i] ^ p.SizeXorKey); }
...
TResult<FByteArray> FSphrCodec::Decode(const FByteArray& bytes) {
    ...
    for (size_t off : {size_t(9), size_t(17), size_t(20)}) {
        if (off < patched.size()) { patched[off] ^= probe.HeaderXorKey; }
    }
    for (size_t i = 0; i < 4; ++i) { patched[4 + i] ^= probe.SizeXorKey; }
