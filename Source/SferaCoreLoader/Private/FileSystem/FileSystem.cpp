...
namespace Sfera {
FFileSystem::FFileSystem(FPath root) { SetRoot(root.empty() ? PathUtils::GetExecutableDirectory() : std::move(root)); }
...
void FFileSystem::BuildCatalog(FLogger* logger) {
    ...
    if (!std::filesystem::exists(Root)) {
        if (logger) { logger->Warning("FileSystem root does not exist: " + Root.string()); }
        return;
    }
    ...
    if (logger) { logger->Info("FileSystem cataloged " + std::to_string(Files.size()) + " files under executable directory"); }
}

std::optional<FPath> FFileSystem::Resolve(std::string_view logicalPath) const {
    FPath input{std::string(logicalPath)};
