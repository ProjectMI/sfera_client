...
float F32Le(const FByteArray& bytes, size_t offset) { uint32 raw = U32Le(bytes, offset); float value = 0.0f; static_assert(sizeof(value) == sizeof(raw)); std::memcpy(&value, &raw, sizeof(value)); return value; }
...
bool SkipCString(const FByteArray& bytes, size_t& offset) { while (offset < bytes.size()) { if (bytes[offset++] == 0) { return true; } } return false; }
FStatus LoadSfnGlyphs(const FByteArray& bytes, const std::string& sourceName, int32& lineHeight, int32& baseline, std::array<FD3D9BitmapGlyph, 224>& glyphs, int32 textureWidth, int32 textureHeight) {
    ...
    for (size_t i = 0; i < glyphs.size(); ++i) { size_t rec = offset + i * 28; FD3D9BitmapGlyph glyph; int32 cellWidth = static_cast<int32>(I16Le(bytes, rec)); int32 visibleWidth = static_cast<int32>(I16Le(bytes, rec + 8)); glyph.Advance = visibleWidth > 0 ? visibleWidth : cellWidth; glyph.Height = static_cast<int32>(I16Le(bytes, rec + 2)); glyph.XOffset = static_cast<int32>(I16Le(bytes, rec + 4)); glyph.YOffset = static_cast<int32>(I16Le(bytes, rec + 6)); float u1 = F32Le(bytes, rec + 12); float v1 = F32Le(bytes, rec + 16); float u2 = F32Le(bytes, rec + 20); float v2 = F32Le(bytes, rec + 24); glyph.SourceX = static_cast<int32>(std::lround(u1 * textureWidth)); glyph.SourceY = static_cast<int32>(std::lround(v1 * textureHeight)); glyph.SourceW = static_cast<int32>(std::lround((u2 - u1) * textureWidth)); glyph.SourceH = static_cast<int32>(std::lround((v2 - v1) * textureHeight)); glyphs[i] = glyph; }
    return FStatus::Ok();
}
TResult<IDirect3DTexture9*> LoadFontDds(IDirect3DDevice9* device, const FByteArray& bytes, const std::string& sourceName, int32& outWidth, int32& outHeight) {
    ...
    uint32 headerSize = U32Le(bytes, 4); int32 height = static_cast<int32>(U32Le(bytes, 12)); int32 width = static_cast<int32>(U32Le(bytes, 16)); uint32 pfSize = U32Le(bytes, 76); uint32 pfFlags = U32Le(bytes, 80); uint32 bits = U32Le(bytes, 88); uint32 rMask = U32Le(bytes, 92); uint32 gMask = U32Le(bytes, 96); uint32 bMask = U32Le(bytes, 100); uint32 aMask = U32Le(bytes, 104);
    ...
    bool hasAlpha = bits == 32 && (pfFlags & 0x1U) != 0 && aMask == 0xFF000000U; size_t bpp = bits / 8; size_t sourceStride = static_cast<size_t>(width) * bpp; size_t imageSize = sourceStride * static_cast<size_t>(height);
    if (!HasRange(bytes, 128, imageSize)) { return FStatus::Error(EStatusCode::InvalidData, "truncated DDS pixels: " + sourceName); }
    IDirect3DTexture9* texture = nullptr; HRESULT hr = device->CreateTexture(static_cast<UINT>(width), static_cast<UINT>(height), 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &texture, nullptr);
...
FD3D9BitmapFont& FD3D9BitmapFont::operator=(FD3D9BitmapFont&& other) noexcept { if (this != &other) { Release(); LineHeightValue = other.LineHeightValue; BaselineValue = other.BaselineValue; TextureWidth = other.TextureWidth; TextureHeight = other.TextureHeight; Texture = other.Texture; Glyphs = other.Glyphs; other.LineHeightValue = 0; other.BaselineValue = 0; other.TextureWidth = 0; other.TextureHeight = 0; other.Texture = nullptr; } return *this; }
FD3D9BitmapFont::~FD3D9BitmapFont() { Release(); }
void FD3D9BitmapFont::Release() { if (Texture) { Texture->Release(); Texture = nullptr; } TextureWidth = 0; TextureHeight = 0; LineHeightValue = 0; BaselineValue = 0; Glyphs = {}; }
int32 FD3D9BitmapFont::MeasureCodepageText(const std::vector<uint8>& text) const { int32 width = 0; for (uint8 ch : text) { if (ch >= 32) { width += Glyphs[static_cast<size_t>(ch - 32)].Advance; } } return width; }
std::vector<uint8> FD3D9BitmapFont::EncodeUtf8ToCp1251(std::string_view text) const { std::wstring wide = Utf8ToWide(text); if (wide.empty()) { return {}; } int needed = WideCharToMultiByte(1251, 0, wide.c_str(), static_cast<int>(wide.size()), nullptr, 0, "?", nullptr); std::vector<uint8> out(static_cast<size_t>(std::max(0, needed))); if (needed > 0) { WideCharToMultiByte(1251, 0, wide.c_str(), static_cast<int>(wide.size()), reinterpret_cast<char*>(out.data()), needed, "?", nullptr); } return out; }
TResult<FD3D9BitmapFont> FD3D9BitmapFont::Load(IDirect3DDevice9* device, const FResourceManager& resources, std::string_view fontName) {
    std::string name(fontName); std::string sfnName = ResolveSfnName(resources, name); std::string ddsName = ResolveDdsName(resources, name);
    ...
    font.Texture = texture.Value(); font.TextureWidth = texW; font.TextureHeight = texH;
...
const FD3D9BitmapFont* FD3D9BitmapFontCatalog::GetFont(IDirect3DDevice9* device, const FResourceManager& resources, int32 fontIndex, FLogger* logger) {
    ...
    if (!loaded.IsOk()) { if (logger) { logger->Warning("UI bitmap font load failed: requested=" + std::to_string(requestedIndex) + " mapped=" + std::to_string(mappedIndex) + " name=" + name + " - " + loaded.Status().Message()); } LoadedFonts.emplace(key, nullptr); return nullptr; }
    auto font = std::make_unique<FD3D9BitmapFont>(std::move(loaded.Value())); const FD3D9BitmapFont* out = font.get(); LoadedFonts.emplace(key, std::move(font)); if (logger) { logger->Info("UI bitmap font loaded: requested=" + std::to_string(requestedIndex) + " mapped=" + std::to_string(mappedIndex) + " name=" + name); } return out;
}
}
