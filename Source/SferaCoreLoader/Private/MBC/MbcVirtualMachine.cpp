...
FStatus FMbcVirtualMachine::LoadModule(const FMbcModule* module)
{
    LoadedModule = module;
    SingleModuleProject = {};
    ...
    if (module)
    {
        SingleModuleProject.Modules().push_back(*module);
        SingleModuleProject.BuildLinker();
        Project = &SingleModuleProject;
    }

    return ValidateLoadedModule();
}
...
FMbcExecutionResult FMbcVirtualMachine::RunModuleProgram(std::string_view moduleName, std::string_view programName, FMbcExecutionLimits limits)
{
    ...
    for (uint32 i = 0; i < Project->Modules().size(); ++i)
    {
        if (&Project->Modules()[i] == module)
        {
            moduleIndex = i;
            break;
        }
...
FMbcExecutionResult FMbcVirtualMachine::RunFrame(FMbcRuntimeFrame& frame, FMbcExecutionLimits limits)
{
    ...
    while (frame.Running && frame.Pc < module.Code().size() && result.Instructions < limits.MaxInstructions)
    {
        ...
        if (decoded.Terminal && frame.Pc == oldPc + std::max<uint32>(decoded.Length, 1))
        {
            frame.Running = false;
        }
    }
    ...
    {
        result.Status = FStatus::Error(EStatusCode::RuntimeError, "MBC instruction limit reached");
    }
...
FStatus FMbcVirtualMachine::ExecuteDecoded(FMbcRuntimeFrame& frame, const FMbcDecodedOpcode& decoded)
{
    ...
    if (m == "push_imm32")
    {
        uint8 typ = static_cast<uint8>(uop("type"));

        if (typ == Mbc::TypeFloat)
        {
            Push(frame, FMbcValue::Float(Mbc::FloatFromU32(uop("value_u32"))));
        }
        else
        {
            Push(frame, FMbcValue::Int(iop("value_i32")));
        }
    ...
    if (m == "add" || m == "sub" || m == "mul" || m == "div" || m == "mod")
    {
        FMbcValue b = Pop(frame);
        FMbcValue a = Pop(frame);

        if (a.Type == Mbc::TypeFloat || b.Type == Mbc::TypeFloat)
        {
            float av = a.Type == Mbc::TypeFloat ? a.FloatValue : static_cast<float>(a.IntValue);
            float bv = b.Type == Mbc::TypeFloat ? b.FloatValue : static_cast<float>(b.IntValue);

            if (m == "add")
            {
                Push(frame, FMbcValue::Float(av + bv));
            }
            else if (m == "sub")
            {
                Push(frame, FMbcValue::Float(av - bv));
            ...
            {
                Push(frame, FMbcValue::Float(bv == 0.0f ? 0.0f : av / bv));
            }
            else
            {
                Push(frame, FMbcValue::Int(static_cast<int32>(av) % std::max<int32>(1, static_cast<int32>(bv))));
            }
        }
        else
        {
            ...
            if (m == "add")
            {
                Push(frame, FMbcValue::Int(av + bv));
            }
            else if (m == "sub")
            {
                Push(frame, FMbcValue::Int(av - bv));
            ...
            {
                Push(frame, FMbcValue::Int(bv == 0 ? 0 : av / bv));
            }
            else
            {
                Push(frame, FMbcValue::Int(bv == 0 ? 0 : av % bv));
    ...
    if (m == "eq" || m == "ne" || m == "gt" || m == "lt" || m == "ge" || m == "le")
    {
        FMbcValue b = Pop(frame);
        FMbcValue a = Pop(frame);
        float av = a.Type == Mbc::TypeFloat ? a.FloatValue : static_cast<float>(a.IntValue);
        float bv = b.Type == Mbc::TypeFloat ? b.FloatValue : static_cast<float>(b.IntValue);
        bool r = m == "eq" ? av == bv : m == "ne" ? av != bv : m == "gt" ? av > bv : m == "lt" ? av < bv : m == "ge" ? av >= bv : av <= bv;
        Push(frame, FMbcValue::Int(r ? 1 : 0));
        return FStatus::Ok();
    }
    ...
    {
        bool t = Truthy(Pop(frame));

        if (t)
        {
            Push(frame, FMbcValue::Int(1));
            frame.Pc = uop("target");
        }
    ...
    if (m == "return" || m == "return_local")
    {
        ...
        frame.Pc = frame.ReturnStack.back();
    ...
    if (m == "import_stub_u32" || m == "call_linked_function")
    {
        const FMbcFunctionSymbol* sym = Project->Linker().SymbolAt(frame.ModuleIndex, frame.Pc - std::max<uint32>(decoded.Length, 1));

        if (sym)
        {
            const FMbcNativeImport* nativeSpec = Project->Linker().ResolveNative(sym->Name);

            if (nativeSpec)
            {
                FMbcNativeContext ctx;
                ctx.Vm = this;
                ctx.Data = &frame.Data;
                ctx.Name = sym->Name;

                if (const FMbcNative* fn = Registry.Find(sym->Name))
                {
                    FStatus st = (*fn)(ctx);

                    if (ctx.ReturnValue)
                    {
                        Push(frame, *ctx.ReturnValue);
                    }

                    return st;
                }

                if (nativeSpec->PushesValue)
                {
                    Push(frame, FMbcValue::Int(0));
                }
            ...
            const FMbcFunctionSymbol* target = Project->Linker().ResolveImport(*sym);

            if (target && target->ModuleIndex == frame.ModuleIndex)
            {
                frame.ReturnStack.push_back(frame.Pc);
                frame.Pc = target->CodeOffset;
                return FStatus::Ok();
            }
        }

        AddEvent("unresolved_import", m, frame);
        frame.Running = false;
        return FStatus::Ok();
    }

    AddEvent("unimplemented_opcode", m, frame, decoded.Known ? "known opcode pending exact runtime handler" : "unknown opcode trap");
    return FStatus::Ok();
}
FStatus FMbcVirtualMachine::ExecuteBuiltin(FMbcRuntimeFrame& frame, const FMbcDecodedOpcode& decoded)
{
    ...
    if (it != decoded.Operands.end())
    {
        sub = NumericParse::UInt32Or(it->second);
    }
    ...
    case 3:
        {
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Float(std::sin(a.FloatValue)));
            return FStatus::Ok();
        }
    case 4:
        {
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Float(std::cos(a.FloatValue)));
            return FStatus::Ok();
        }
    case 5:
        {
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Float(std::fabs(a.FloatValue)));
            return FStatus::Ok();
        }
    case 6:
        {
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Int(std::abs(a.IntValue)));
            return FStatus::Ok();
        }
    case 23:
        {
            Push(frame, FMbcValue::Int(static_cast<int32>(frame.Stack.size())));
            return FStatus::Ok();
        }
    case 25: case 27:
        {
    ...
    case 37: case 75: case 76: case 137: case 147:
        {
            Push(frame, FMbcValue::Int(0));
            return FStatus::Ok();
        }
    ...
    case 106:
        {
            Push(frame, FMbcValue::Float(0.0f));
            return FStatus::Ok();
        }
    case 119:
        {
    ...
    case 138:
        {
            FMbcValue b = Pop(frame);
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Int(a.IntValue & b.IntValue));
            return FStatus::Ok();
        }
    case 139:
        {
    ...
    case 141:
        {
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Int(~a.IntValue));
            return FStatus::Ok();
        }
    case 142:
        {
            FMbcValue b = Pop(frame);
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Int(a.IntValue << (b.IntValue & 31)));
            return FStatus::Ok();
        }
    case 143:
        {
    ...
    case 144:
        {
            FMbcValue b = Pop(frame);
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Int(a.IntValue & ~(1 << (b.IntValue & 31))));
            return FStatus::Ok();
        }
    case 145:
        {
    ...
    case 146:
        {
            FMbcValue b = Pop(frame);
            FMbcValue a = Pop(frame);
            Push(frame, FMbcValue::Int((a.IntValue >> (b.IntValue & 31)) & 1));
            return FStatus::Ok();
        }
    default:
        {
            if (const FMbcNative* fn = Registry.Find(name))
            {
                FMbcNativeContext ctx;
                ctx.Vm = this;
                ctx.Data = &frame.Data;
                ctx.Name = name;
                ctx.Args = frame.Stack;
                FStatus st = (*fn)(ctx);
                ...
                AddEvent("builtin_bridge", name, frame, ctx.Commentary);
                return st;
            }

            bool pushes = builtin && (std::string(builtin->Semantic).find("push") != std::string::npos || std::string(builtin->Mnemonic).find("get") != std::string::npos || std::string(builtin->Mnemonic).find("query") != std::string::npos);
...
void FMbcVirtualMachine::Push(FMbcRuntimeFrame& frame, FMbcValue value)
{
    frame.Stack.push_back(value);
}
FMbcValue FMbcVirtualMachine::Pop(FMbcRuntimeFrame& frame)
{
    if (frame.Stack.empty())
    {
        return FMbcValue::Int(0);
    }

    FMbcValue v = frame.Stack.back();
    frame.Stack.pop_back();
    return v;
}
bool FMbcVirtualMachine::Truthy(const FMbcValue& value) const
{
    if (value.Type == Mbc::TypeFloat)
    {
        return value.FloatValue != 0.0f;
    }

    if (value.Type == Mbc::TypeSlice)
    {
        return value.Slice.Length != 0;
    }

    return value.IntValue != 0;
}
void FMbcVirtualMachine::AddEvent(std::string kind, std::string name, const FMbcRuntimeFrame& frame, std::string commentary)
{
    FMbcRuntimeEvent e;
    e.Kind = std::move(kind);
    e.Name = std::move(name);
    e.ModuleIndex = frame.ModuleIndex;
    e.ProgramIndex = frame.ProgramIndex;
    e.CodeOffset = frame.Pc;
    e.Commentary = std::move(commentary);
