...
namespace Sfera {
static std::string LowerPathString(FPath path) { std::string s = path.generic_string(); std::transform(s.begin(), s.end(), s.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); }); return s; }
static uint16 ReadU16LE(const FByteArray& bytes, size_t offset) { if (offset + 2 > bytes.size()) { return 0; } return static_cast<uint16>(bytes[offset] | (bytes[offset + 1] << 8)); }
static uint32 ReadU32LE(const FByteArray& bytes, size_t offset) { if (offset + 4 > bytes.size()) { return 0; } return static_cast<uint32>(bytes[offset] | (bytes[offset + 1] << 8) | (bytes[offset + 2] << 16) | (bytes[offset + 3] << 24)); }
static int32 ReadI32LE(const FByteArray& bytes, size_t offset) { return static_cast<int32>(ReadU32LE(bytes, offset)); }
static float ReadFloatLE(const FByteArray& bytes, size_t offset) { float value = 0.0f; if (offset + sizeof(float) <= bytes.size()) { std::memcpy(&value, bytes.data() + offset, sizeof(float)); } return value; }
...
static void ExpandBounds(FBox2& box, FVector2 point, bool& initialized) { if (!initialized) { box.Min = point; box.Max = point; initialized = true; return; } box.Min.X = std::min(box.Min.X, point.X); box.Min.Y = std::min(box.Min.Y, point.Y); box.Max.X = std::max(box.Max.X, point.X); box.Max.Y = std::max(box.Max.Y, point.Y); }
static std::string ReadFixedCString(const FByteArray& bytes, size_t offset, size_t maxLen) { std::string out; for (size_t i = 0; i < maxLen && offset + i < bytes.size(); ++i) { char c = static_cast<char>(bytes[offset + i]); if (c == '\0') { break; } out.push_back(c); } while (!out.empty() && std::isspace(static_cast<unsigned char>(out.back()))) { out.pop_back(); } return out; }
...
static std::string StripKnownExtension(std::string s) { size_t slash = s.find_last_of("/\\"); size_t dot = s.find_last_of('.'); if (dot != std::string::npos && (slash == std::string::npos || dot > slash)) { s.erase(dot); } return s; }
...
FStatus FWorldScene::LoadBootstrapScene(FLogger* logger) {
    ...
    if (logger) { auto s = Stats(); logger->Info("WorldScene bootstrap: landscape_resources=" + std::to_string(s.PatchCount) + ", zones=" + std::to_string(s.ZoneCount) + ", contours=" + std::to_string(s.ContourCount) + ", map_present=" + std::to_string(s.MapPresentCells) + ", grass=" + std::to_string(s.GrassPatchCount) + ", snow_points=" + std::to_string(s.SnowPointCount) + ", weather_lines=" + std::to_string(s.WeatherLineCount) + ", sky_lines=" + std::to_string(s.SkyLineCount)); }
    return FStatus::Ok();
}

FStatus FWorldScene::ReloadZoning(FLogger* logger) { return ZoningManager.LoadDefault(Resources, logger); }
void FWorldScene::SyncObject(uint32 objectId, FVector3 position, float radius, std::string tag) { FBox2 b; b.Min = {position.X - radius, position.Z - radius}; b.Max = {position.X + radius, position.Z + radius}; Spatial.Insert(objectId, b, std::move(tag)); }
void FWorldScene::RemoveObject(uint32 objectId) { Spatial.Remove(objectId); }
std::vector<uint32> FWorldScene::QueryObjects(FBox2 area) const { return Spatial.Query(area); }
FWorldSceneStats FWorldScene::Stats() const { FWorldSceneStats stats; stats.PatchCount = PatchRecords.size(); stats.TerrainSizeRecordCount = TerrainRecords.size(); stats.MicrotextureRecordCount = MicrotextureRecords.size(); stats.ZoneCount = ZoningManager.Zones().size(); stats.BinaryBlobCount = BinaryBlobRecords.size(); stats.WeatherLineCount = WeatherProfile.Lines.size(); stats.SkyLineCount = SkyProfile.Lines.size(); stats.SpatialObjectCount = Spatial.Count(); stats.ContourCount = Contours.Records.size(); stats.MapCellCount = MapGrid.Cells.size(); stats.MapPresentCells = MapGrid.PresentCells; stats.GrassPatchCount = GrassDatabase.Patches.size(); stats.SnowPointCount = SnowPath.CandidatePointCount; return stats; }

std::string FWorldScene::NormalizeResourceKey(std::string_view text) { std::string s(text); std::replace(s.begin(), s.end(), '\\', '/'); std::transform(s.begin(), s.end(), s.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); }); while (!s.empty() && (s.front() == '/' || std::isspace(static_cast<unsigned char>(s.front())))) { s.erase(s.begin()); } while (!s.empty() && std::isspace(static_cast<unsigned char>(s.back()))) { s.pop_back(); } return s; }
...
void FWorldScene::BuildPatchLookup() {
    ...
    for (size_t i = 0; i < PatchRecords.size(); ++i) {
        ...
        if (!stemKey.empty()) { PatchByName[stemKey] = i; PatchByName["landscape/" + stemKey] = i; }
        std::string pathNoExt = StripKnownExtension(pathKey);
        if (!pathNoExt.empty()) { PatchByName[pathNoExt] = i; }
        if (patch.HasPatchCoords) { PatchByCoord[MakeCoordKey(patch.PatchX, patch.PatchZ)] = i; }
    }
}
...
void FWorldScene::BuildTerrainLookup() {
    ...
    for (size_t i = 0; i < TerrainRecords.size(); ++i) {
        ...
        if (!stemKey.empty()) { TerrainByName[stemKey] = i; TerrainByName["landscape/" + stemKey] = i; }
        std::string pathNoExt = StripKnownExtension(pathKey);
        if (!pathNoExt.empty()) { TerrainByName[pathNoExt] = i; }
    }
    for (size_t i = 0; i < MicrotextureRecords.size(); ++i) {
        ...
        if (!stemKey.empty()) { MicrotextureByName[stemKey] = i; MicrotextureByName["landscape/" + stemKey] = i; }
        std::string pathNoExt = StripKnownExtension(pathKey);
        if (!pathNoExt.empty()) { MicrotextureByName[pathNoExt] = i; }
    }
}
...
const FWorldPatchRecord* FWorldScene::FindPatchByName(std::string_view name) const {
    ...
    if (stem != key) { it = PatchByName.find(stem); if (it != PatchByName.end()) { return &PatchRecords[it->second]; } }
...
const FWorldTerrainSizeRecord* FWorldScene::FindTerrainSizeByName(std::string_view name) const {
    ...
    std::string stem = StripKnownExtension(key);
    if (stem != key) { it = TerrainByName.find(stem); if (it != TerrainByName.end()) { return &TerrainRecords[it->second]; } }
    std::string withLandscape = "landscape/" + key;
...
const FWorldMicrotextureRecord* FWorldScene::FindMicrotextureByName(std::string_view name) const {
    ...
    std::string stem = StripKnownExtension(key);
    if (stem != key) { it = MicrotextureByName.find(stem); if (it != MicrotextureByName.end()) { return &MicrotextureRecords[it->second]; } }
    std::string withLandscape = "landscape/" + key;
...
const FWorldPatchRecord* FWorldScene::FindPatchByCoord(int32 patchX, int32 patchZ) const { auto it = PatchByCoord.find(MakeCoordKey(patchX, patchZ)); if (it == PatchByCoord.end()) { return nullptr; } return &PatchRecords[it->second]; }
...
void FWorldScene::CatalogLandscapeResources(FLogger* logger) {
    for (const auto& item : Resources.Catalog().All()) {
        ...
        if (std::regex_search(pathText, match, coordRegex) && match.size() >= 3) { int32 px = 0; int32 pz = 0; if (NumericParse::TryParseInt32Strict(match[1].str(), px) && NumericParse::TryParseInt32Strict(match[2].str(), pz)) { record.PatchX = px; record.PatchZ = pz; record.HasPatchCoords = true; } }
        ...
        if (IsLandscapeSizPath(lower)) {
            ...
            if (blob.IsOk() && blob.Value().Bytes.size() >= 8) { terrain.RawWidth = ReadU32LE(blob.Value().Bytes, 0); terrain.RawHeight = ReadU32LE(blob.Value().Bytes, 4); terrain.HasRawDimensions = true; }
    ...
    BuildPatchLookup();
    BuildTerrainLookup();
    if (logger) { logger->Info("WorldScene cataloged landscape resources: " + std::to_string(PatchRecords.size()) + ", terrain_size_records=" + std::to_string(TerrainRecords.size()) + ", microtextures=" + std::to_string(MicrotextureRecords.size()) + ", coord_index=" + std::to_string(PatchByCoord.size())); }
}
...
void FWorldScene::LoadRuntimeBinaries(FLogger* logger) {
    ...
    LoadSnowPath(logger);
    if (logger) { logger->Info("WorldScene runtime binaries: contours=" + std::to_string(Contours.Records.size()) + ", map_cells=" + std::to_string(MapGrid.PresentCells) + ", grass_maps=" + std::to_string(GrassDatabase.Patches.size()) + ", snow_points=" + std::to_string(SnowPath.CandidatePointCount)); }
}
...
void FWorldScene::LoadContours(FLogger* logger) {
    ...
    if (count > 200000 || expected > bytes.size()) { count = static_cast<uint32>(bytes.size() / recordSize); }
    ...
    for (uint32 i = 0; i < count; ++i) {
        ...
        for (int32 p = 0; p < rec.PointCount; ++p) { rec.ForwardLinks.push_back(ReadI32LE(bytes, base + 0x218 + static_cast<size_t>(p) * 4)); rec.BackLinks.push_back(ReadI32LE(bytes, base + 0x318 + static_cast<size_t>(p) * 4)); }
        if (!localInit) { rec.Bounds.Min = {static_cast<float>(rec.SortKey), 0.0f}; rec.Bounds.Max = rec.Bounds.Min; ++Contours.InvalidRecords; }
        Contours.Records.push_back(std::move(rec));
    }
    if (logger) { logger->Info("WorldScene contours loaded: records=" + std::to_string(Contours.Records.size()) + ", invalid=" + std::to_string(Contours.InvalidRecords) + ", record_size=0x418"); }
}
...
void FWorldScene::LoadMapGrid(FLogger* logger) {
        ...
        for (uint32 x = 0; x < width; ++x) {
            ...
            if (cell.Present) {
                ...
                if (const FWorldTerrainSizeRecord* terrain = FindTerrainSizeByName(cell.TileName)) {
                    ...
                    if (fallbackSamples.size() < 8) { fallbackSamples.push_back(cell.TileName); }
                } else {
                    ++MapGrid.MissingTileRefs;
                    if (unresolvedSamples.size() < 8) { unresolvedSamples.push_back(cell.TileName); }
                }
            }
    ...
    if (logger) {
        ...
        if (!fallbackSamples.empty()) {
            std::string joined;
            for (size_t i = 0; i < fallbackSamples.size(); ++i) { if (i) { joined += ", "; } joined += fallbackSamples[i]; }
            logger->Warning("WorldScene map tile fallback samples: " + joined);
        }
        if (!unresolvedSamples.empty()) {
            std::string joined;
            for (size_t i = 0; i < unresolvedSamples.size(); ++i) { if (i) { joined += ", "; } joined += unresolvedSamples[i]; }
...
void FWorldScene::LoadGrassMaps(FLogger* logger) {
    ...
    for (const auto& item : Resources.Catalog().All()) {
        ...
        for (uint8 b : blob.Value().Bytes) { if (b != 0) { ++patch.NonZeroCells; patch.MaxValue = std::max<uint8>(patch.MaxValue, b); } }
        GrassDatabase.Patches.push_back(std::move(patch));
    }
    GrassDatabase.Loaded = !GrassDatabase.Patches.empty();
    if (logger) { logger->Info("WorldScene grass maps loaded: " + std::to_string(GrassDatabase.Patches.size()) + (GrassDatabase.Patches.empty() ? "" : ", tile_bytes=0x10000")); }
}
...
void FWorldScene::LoadSnowPath(FLogger* logger) {
    ...
    if (bytes.size() >= 8 && bytes.size() % 8 == 0) {
        ...
        for (size_t i = 0; i + 8 <= bytes.size(); i += 8) { float x = ReadFloatLE(bytes, i); float z = ReadFloatLE(bytes, i + 4); if (ReasonableWorldFloat(x) && ReasonableWorldFloat(z)) { ExpandBounds(SnowPath.Bounds, {x, z}, init); } }
    } else { SnowPath.CandidatePointCount = static_cast<uint32>(bytes.size() / 4); }
    if (logger) { logger->Info("WorldScene snow path loaded: bytes=" + std::to_string(SnowPath.Size) + ", candidate_points=" + std::to_string(SnowPath.CandidatePointCount)); }
}

void FWorldScene::LoadTextProfiles(FLogger* logger) {
    auto loadProfile = [&](const char* logicalName, auto& profile) { auto blob = Resources.Load(logicalName); if (!blob.IsOk()) { return false; } profile.SourceName = logicalName; std::string text(blob.Value().Bytes.begin(), blob.Value().Bytes.end()); ParseKeyValueText(text, profile.Lines, profile.Values); return true; };
    if (!loadProfile("landscape/weather.txt", WeatherProfile)) { loadProfile("landscape_hr/weather_hr.txt", WeatherProfile); }
    if (!loadProfile("sky.txt", SkyProfile)) { loadProfile("landscape_hr/sky_hr.txt", SkyProfile); }
    for (const auto& pair : SkyProfile.Values) {
        const std::string& key = pair.first;
        if (key.find("nsky") == std::string::npos && key.find("tsky") == std::string::npos) { continue; }
    }
    if (logger) { logger->Info("WorldScene text profiles: weather=" + std::to_string(WeatherProfile.Lines.size()) + ", sky=" + std::to_string(SkyProfile.Lines.size())); }
}
...
void FWorldScene::RunAsmSelfTest(FLogger* logger) const {
    ...
    bool grassOk = GrassDatabase.Patches.empty() || std::all_of(GrassDatabase.Patches.begin(), GrassDatabase.Patches.end(), [](const FWorldGrassPatch& patch) { return patch.Size == 0x10000; });
    std::string sample;
    if (const FWorldMapCell* cell = MapGrid.Find(0, 0)) { sample = cell->TileName; }
...
FWorldBinaryBlobInfo FWorldScene::AnalyzeBinaryBlob(std::string logicalName, const FByteArray& bytes) {
    ...
    if (bytes.size() >= 16 && bytes.size() % 16 == 0) { info.CandidateRecordCount = static_cast<uint32>(bytes.size() / 16); info.Interpretation = "candidate vec4/rect table"; }
    else if (bytes.size() >= 8 && bytes.size() % 8 == 0) { info.CandidateRecordCount = static_cast<uint32>(bytes.size() / 8); info.Interpretation = "candidate point/edge table"; }
    else { info.CandidateRecordCount = 0; info.Interpretation = "opaque world binary"; }
    if (bytes.size() >= 8 && bytes.size() % 8 == 0) { bool initialized = false; for (size_t i = 0; i + 8 <= bytes.size() && i < 8 * 1024; i += 8) { float x = ReadFloatLE(bytes, i); float y = ReadFloatLE(bytes, i + 4); if (!ReasonableWorldFloat(x) || !ReasonableWorldFloat(y)) { continue; } ExpandBounds(info.Bounds, {x, y}, initialized); } }
    return info;
}
...
void FWorldScene::ParseKeyValueText(std::string_view text, std::vector<std::string>& lines, std::unordered_map<std::string, std::string>& values) {
    ...
    while (std::getline(input, line)) {
        ...
        if (split != std::string::npos) { std::string key = NormalizeResourceKey(line.substr(0, split)); std::string value = line.substr(split + 1); values[key] = value; }
    }
}
}
