...
void FMbcNativeRegistry::RegisterNoOp(std::string name, bool pushesValue) { Register(std::move(name), [pushesValue](FMbcNativeContext& ctx) { ctx.Commentary = "native boundary stub"; if (pushesValue) { ctx.ReturnValue = FMbcValue::Int(0); } return FStatus::Ok(); }); }
