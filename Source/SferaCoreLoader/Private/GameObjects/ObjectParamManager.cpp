...
FStatus FObjectParamManager::Open(const FResourceManager& resources, std::string_view logicalName, FLogger* logger) {
    ...
    if (kind == EObjectParamSourceKind::None) { kind = EObjectParamSourceKind::ExplicitDocument; }
...
FStatus FObjectParamManager::OpenKnownConfigs(const FResourceManager& resources, FLogger* logger) {
    ...
    for (const auto& item : resources.Catalog().All()) {
        ...
        ++loaded;
        switch (kind) {
            case EObjectParamSourceKind::ModelParamTable: ++modelTables; break;
            case EObjectParamSourceKind::EntityParamFile: ++entityFiles; break;
            case EObjectParamSourceKind::GroupTable: ++groupTables; break;
            default: break;
        }
    }
...
FObjectParamValue FObjectParamManager::ParseValue(std::string value) {
    ...
    if (hasFloatMarker) { float parsed = 0.0f; if (NumericParse::TryParseFloatStrict(trimmed, parsed)) { return FObjectParamValue::Float(parsed); } }
    int32 parsedInt = 0;
...
FObjectParamSource FObjectParamManager::IndexDocument(const FConfigDocument& doc, EObjectParamSourceKind kind, FLogger* logger) {
    ...
    if (kind == EObjectParamSourceKind::ModelParamTable) {
        for (const auto& entry : doc.Entries()) {
            ...
            if (descriptor.Archetype.empty()) { descriptor.Archetype = name; descriptor.SourceConfig = doc.SourceName(); }
        ...
        for (const auto& entry : doc.Entries()) {
            ...
            if (descriptor.Archetype.empty()) { descriptor.Archetype = objectName; descriptor.SourceConfig = doc.SourceName(); }
        ...
        for (const auto& entry : doc.Entries()) {
            ...
            if (descriptor.Archetype.empty()) { descriptor.Archetype = objectKey; descriptor.SourceConfig = doc.SourceName(); }
            std::string paramName = entry.Key;
            size_t dot = paramName.find_first_of('.');
            if (dot != std::string::npos && dot + 1 < paramName.size()) { paramName = paramName.substr(dot + 1); }
