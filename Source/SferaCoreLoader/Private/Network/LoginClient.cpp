...
class FWsaSession {
public:
    FWsaSession() { OkValue = WSAStartup(MAKEWORD(2, 2), &Data) == 0; }
    FWsaSession(const FWsaSession&) = delete;
    FWsaSession& operator=(const FWsaSession&) = delete;
    ~FWsaSession() { if (OkValue) { WSACleanup(); } }
...
public:
    ...
    FSocketHandle(FSocketHandle&& other) noexcept { Socket = other.Socket; other.Socket = INVALID_SOCKET; }
    FSocketHandle& operator=(FSocketHandle&& other) noexcept { if (this != &other) { Reset(); Socket = other.Socket; other.Socket = INVALID_SOCKET; } return *this; }
    ~FSocketHandle() { Reset(); }
    explicit operator bool() const { return Socket != INVALID_SOCKET; }
    SOCKET Get() const { return Socket; }
    void Reset() { if (Socket != INVALID_SOCKET) { closesocket(Socket); Socket = INVALID_SOCKET; } }
private:
...
FSocketHandle ConnectSocket(const std::string& host, int32 port, int32 timeoutMs, std::string& error) {
    ...
    if (!connected) { error = WsaErrorText("connect failed"); }
    return connected;
}
...
std::vector<uint8> ToCp1251(const std::wstring& text) {
    ...
    if (required > 0) { WideCharToMultiByte(1251, 0, text.c_str(), static_cast<int>(text.size()), reinterpret_cast<char*>(out.data()), required, "?", nullptr); }
    return out;
}
...
std::wstring FromCp1251(const std::vector<uint8>& bytes) {
    ...
    if (required > 0) { MultiByteToWideChar(1251, 0, reinterpret_cast<const char*>(bytes.data()), static_cast<int>(bytes.size()), out.data(), required); }
    return out;
}
...
std::vector<uint8> BuildLegacyPacket(uint16 opcode, const std::vector<uint8>& payload, uint16& sequence, uint16 xorKey) {
    ...
    int16 sum = 0;
    for (size_t i = 4; i < packet.size(); ++i) { sum = static_cast<int16>(sum + static_cast<int8>(packet[i])); }
...
std::vector<uint8> EncodeClientPacketForSphereEmu(const std::vector<uint8>& decoded) {
    constexpr uint8 encodingMask[] = {0x4B, 0x0D, 0xEF, 0x60, 0xC9, 0x9A, 0x70, 0x0E, 0x03};
...
std::array<uint8, 5> BuildCharacterCreationBytes(const FCharacterCreationAppearance& appearance) {
    ...
    auto modelValue = [modelBase](int32 localIndex) { return modelBase + std::clamp(localIndex, 0, 255 - modelBase); };
    ...
    if (appearance.Female) { face = 256 - face; hair = 255 - hair; hairColor = 255 - hairColor; tattoo = 255 - tattoo; }
...
uint8 EncodeCharacterNameSymbol(wchar_t ch) { if (ch >= static_cast<wchar_t>(0x0410) && ch <= static_cast<wchar_t>(0x042f)) { return static_cast<uint8>(129U + static_cast<unsigned>(ch - static_cast<wchar_t>(0x0410)) * 2U); } if (ch >= static_cast<wchar_t>(0x0430) && ch <= static_cast<wchar_t>(0x044f)) { return static_cast<uint8>(193U + static_cast<unsigned>(ch - static_cast<wchar_t>(0x0430)) * 2U); } if (ch == static_cast<wchar_t>(0x0401)) { return static_cast<uint8>(129U + static_cast<unsigned>(static_cast<wchar_t>(0x0415) - static_cast<wchar_t>(0x0410)) * 2U); } if (ch == static_cast<wchar_t>(0x0451)) { return static_cast<uint8>(193U + static_cast<unsigned>(static_cast<wchar_t>(0x0435) - static_cast<wchar_t>(0x0430)) * 2U); } if (ch >= 0x20 && ch <= 0x7f) { return static_cast<uint8>(static_cast<unsigned>(ch) * 2U); } return static_cast<uint8>(L'?' * 2U); }
...
std::vector<uint8> EncodeCharacterNameCheck(const std::wstring& name) {
    ...
    for (size_t i = 1; i <= name.size(); ++i) {
        ...
        if (i == 1 && serverFirstLetterHack && (code & 1U) != 0) { --code; }
...
uint32 ReadBitsLE(const std::vector<uint8>& data, int32 bitOffset, int32 bitCount) {
    uint32 value = 0;
    for (int32 bit = 0; bit < bitCount; ++bit) { const int32 absolute = bitOffset + bit; const size_t byteIndex = static_cast<size_t>(absolute / 8); if (byteIndex >= data.size()) { break; } if ((data[byteIndex] & (1U << (absolute % 8))) != 0) { value |= 1U << bit; } }
    return value;
}
...
void FServerSession::Close() { if (Impl) { Impl->Socket.Reset(); } }
...
FCharacterActionResult FServerSession::CreateCharacter(int32 slot, const std::wstring& name, const FCharacterCreationAppearance& appearance, int32 timeoutMs) {
    ...
    std::ostringstream out;
    if (nameExists) { out << "character name already exists"; }
    else if (namePassed) { out << "created slot " << (std::clamp(slot, 0, 2) + 1); }
    else { out << "character create was not acknowledged"; }
...
FCharacterActionResult FServerSession::DeleteCharacter(int32 slot, int32 timeoutMs) {
    ...
    if (result.Disconnected) { out << "; server closed character session"; } else { out << "; packets=" << result.PacketCount << " bytes=" << result.ByteCount; }
    Impl->Socket.Reset();
...
FLoginProbeResult ProbeLoginServer(const FEndpoint& endpoint, const std::wstring& login, const std::wstring& password, const FCharacterAppearanceRules& appearanceRules, int32 timeoutMs) {
    ...
    if (result.FirstOpcode != 200) { std::ostringstream out; out << "TCP connected; unexpected first opcode " << result.FirstOpcode << " len=" << result.FirstLength; result.Message = out.str(); return result; }
    result.LegacyHandshake = true;
    const uint16 xorKey = ReadU16Le(firstFrame, 8);
    uint16 sequence = static_cast<uint16>((std::rand() % 1000) + 1);
    const std::vector<uint8> connectionAck{3, 0, 0, 0};
    ...
    if (RecvFrame(socket, nextFrame, timeoutMs)) { result.NextLength = static_cast<int32>(nextFrame.size()); result.NextOpcode = ReadU16Le(nextFrame, 2); }
    if (result.NextOpcode == 300 && nextFrame.size() >= 13 && !login.empty() && !password.empty()) {
        ...
        if (RecvFrame(socket, loginResponse, timeoutMs * 3)) {
            ...
            if (LooksLikeCannotConnect(loginResponse)) { std::ostringstream out; out << "Login rejected by server; localId=0x" << std::hex << localId << std::dec; result.Message = out.str(); return result; }
            if (LooksLikeCharacterSelectStart(loginResponse)) {
                ...
                while (extraFrames < 8 && slotFrames < 3 && RecvFrame(socket, extraFrame, 500)) {
                    ...
                    if (LooksLikeCharacterSlot(extraFrame)) { result.CharacterSlots[static_cast<size_t>(slotFrames)] = ParseCharacterSlot(extraFrame, slotFrames, appearanceRules); ++slotFrames; }
                }
    ...
    if (result.NextOpcode != 0) { out << "; next opcode=" << result.NextOpcode << " len=" << result.NextLength; } else { out << "; waiting for server data"; }
    result.Message = out.str();
    return result;
}
}
