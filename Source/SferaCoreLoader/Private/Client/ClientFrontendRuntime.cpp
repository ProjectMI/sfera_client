...
void FClientFrontendRuntime::DrawLoadingFrame(HDC__* dc, const tagRECT& rect) {
    ...
    { std::lock_guard<std::recursive_mutex> lock(UiMutex); stage = Ui.Stage(); progress = Ui.Progress(); lines = Ui.StatusLines(); }
    ...
    RECT fr{x + 2, y + 2, x + std::max(2, fillW) - 2, y + barH - 2};
    if (fillW > 4) { FillRect(dc, &fr, fill); }
    DeleteObject(fill);
    int lineY = y + 34;
    for (const auto& line : lines) { const std::wstring w = Utf8ToWide(line); TextOutW(dc, x, lineY, w.c_str(), static_cast<int>(w.size())); lineY += 20; }
...
void FClientFrontendRuntime::LoadSavedLogin() {
    ...
    if (!login.empty()) { Ui.SetLoginCredentials(login, password, true); AddStatusLine("login: saved credentials loaded"); }
}
...
FStatus FClientFrontendRuntime::CreateShell(const FClientSettings& settings) {
    ...
    Window.SetPaintCallback([this](HDC__* dc, const tagRECT& rect) { DrawLoadingFrame(dc, rect); });
    ShellCreated = true;
    if (Log) { Log->Info("frontend shell created"); }
    return FStatus::Ok();
}

void FClientFrontendRuntime::ShowShell() { if (ShellCreated) { Window.Show(); Window.ClearCloseRequest(); RepaintDirty = true; Window.RequestRepaint(); } }

void FClientFrontendRuntime::ConfigureNetwork(const FClientFrontendDesc& desc) {
    Settings = desc.Settings;
    Endpoint = desc.Endpoint;
    { std::lock_guard<std::mutex> lock(SessionMutex); Session.Configure(Endpoint); }
    if (Endpoint) { AddStatusLine("network: endpoint configured " + Endpoint->Host + ":" + std::to_string(Endpoint->Port)); }
    else { AddStatusLine("network: endpoint not found"); }
}

void FClientFrontendRuntime::SetStage(std::string stage, float progress) {
    { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetStage(std::move(stage), progress); if (Log) { Log->Info("frontend stage: " + Ui.Stage() + ", progress=" + std::to_string(Ui.Progress())); } }
    RepaintDirty = true;
    RequestRepaintThrottled();
}

void FClientFrontendRuntime::AddStatusLine(std::string line) { { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.AddStatusLine(std::move(line)); } RepaintDirty = true; RequestRepaintThrottled(); }

bool FClientFrontendRuntime::PumpUi() { if (!ShellCreated) { return true; } bool open = Window.PumpMessages(); return open && Window.IsOpen(); }
...
FStatus FClientFrontendRuntime::InitializeD3D9(const FResourceManager& resources) {
    ...
    FStatus d3dStatus;
    { std::lock_guard<std::mutex> renderLock(RenderMutex); d3dStatus = RenderDevice.Initialize(Window.Handle(), Window.Width(), Window.Height(), Log); }
    if (!d3dStatus.IsOk()) { return d3dStatus; }
    RenderResources = &resources;
    D3DInitialized.store(true);
    { std::lock_guard<std::recursive_mutex> uiLock(UiMutex); std::lock_guard<std::mutex> renderLock(RenderMutex); RenderDevice.PreloadUiTextures(resources, Ui, Log); }
...
void FClientFrontendRuntime::StartNetworkProbe(const FClientFrontendDesc& desc) {
    ...
    if (desc.TryNetworkProbe) { BeginLoginRequest(); }
}
...
void FClientFrontendRuntime::ProcessUiAction(const std::string& action) {
    ...
    if (action == "save_login_off") { std::lock_guard<std::recursive_mutex> lock(UiMutex); StoreSavedLogin(false, {}, {}); return; }
    ...
    if (action == "character_back_confirmed") { CloseActiveServerSession(); { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetCharacterActionLocked(false); Ui.SetMode(EUiRuntimeMode::Login); Ui.SetStage("login screen ready", 1.0f); } SetStage("login screen ready", 1.0f); return; }
...
void FClientFrontendRuntime::BeginLoginRequest() {
    ...
    std::string login;
    std::string password;
    { std::lock_guard<std::recursive_mutex> lock(UiMutex); login = Ui.ActionState().LoginText; password = Ui.ActionState().PasswordText; }
    ...
    LoginThread = std::thread([this, endpoint, wideLogin, widePassword, rules]() {
        ...
        { std::lock_guard<std::mutex> lock(LoginMutex); PendingLoginResult = std::move(result); }
...
void FClientFrontendRuntime::PollLoginResult() {
    ...
    { std::lock_guard<std::mutex> lock(LoginMutex); if (PendingLoginResult) { result = std::move(PendingLoginResult); PendingLoginResult.reset(); } }
    if (!result) { return; }
    AddStatusLine("network: " + result->Message);
    if (result->CharacterSelectReady) {
        ActiveServerSession = result->Session;
        { std::lock_guard<std::recursive_mutex> lock(UiMutex); const auto& state = Ui.ActionState(); StoreSavedLogin(state.SaveLogin, state.LoginText, state.PasswordText); Ui.SetCharacterSlots(result->CharacterSlots); Ui.SetMode(EUiRuntimeMode::CharacterSelect); Ui.SetStage("character select ready", 1.0f); }
        RepaintDirty = true;
...
void FClientFrontendRuntime::BeginCharacterEnterRequest() {
    ...
    { std::lock_guard<std::recursive_mutex> lock(UiMutex); slot = Ui.SelectedCharacterSlot(); present = Ui.SelectedCharacterPresent(); canCreate = Ui.SelectedCharacterCanCreate(); Ui.SetCharacterActionLocked(true); }
    if (!session || !session->Connected()) { CharacterActionInProgress.store(false); { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetCharacterActionLocked(false); } AddStatusLine("character: server session is not active"); return; }
    if (!present) { CharacterActionInProgress.store(false); { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetCharacterActionLocked(false); } AddStatusLine(canCreate ? "character: creation requires confirmation" : "character: selected slot is unavailable"); return; }
    if (CharacterThread.joinable()) { CharacterThread.join(); }
    ...
    CharacterThread = std::thread([this, session, slot]() {
        ...
        if (selected.Ok) { ack = session->SendIngameAck(2500); ack.Message = selected.Message + "; " + ack.Message; }
        else { ack = selected; }
        { std::lock_guard<std::mutex> lock(CharacterMutex); PendingCharacterResult = std::move(ack); PendingCharacterRefreshResult.reset(); PendingCharacterActionKind = 0; }
        CharacterActionInProgress.store(false);
    });
}
...
void FClientFrontendRuntime::BeginCharacterCreateRequest() {
    ...
    { std::lock_guard<std::recursive_mutex> lock(UiMutex); slot = Ui.SelectedCharacterSlot(); present = Ui.SelectedCharacterPresent(); canCreate = Ui.SelectedCharacterCanCreate(); name = Ui.SelectedCharacterName(); appearance = Ui.SelectedCharacterAppearance(AppearanceRules); loginText = Ui.ActionState().LoginText; passwordText = Ui.ActionState().PasswordText; Ui.SetCharacterActionLocked(true); }
    if (!session || !session->Connected()) { CharacterActionInProgress.store(false); { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetCharacterActionLocked(false); } AddStatusLine("character: server session is not active"); return; }
    if (present || !canCreate) { CharacterActionInProgress.store(false); { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetCharacterActionLocked(false); } AddStatusLine("character: selected slot is not creatable"); return; }
    if (name.empty()) { CharacterActionInProgress.store(false); { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetCharacterActionLocked(false); } AddStatusLine("character: name is empty"); return; }
    if (CharacterThread.joinable()) { CharacterThread.join(); }
    ...
    CharacterThread = std::thread([this, session, slot, name, appearance, wideLogin, widePassword]() mutable {
        ...
        if (created.Ok) { session->Close(); refresh = RefreshCharacterSelectSession(wideLogin, widePassword, 2500); }
        { std::lock_guard<std::mutex> lock(CharacterMutex); PendingCharacterResult = std::move(created); PendingCharacterRefreshResult = std::move(refresh); PendingCharacterActionKind = 1; PendingCharacterSlot = slot; PendingCharacterName = name; PendingCharacterAppearance = appearance; }
        CharacterActionInProgress.store(false);
    });
}
...
void FClientFrontendRuntime::BeginCharacterDeleteRequest() {
    ...
    std::string loginText;
    std::string passwordText;
    { std::lock_guard<std::recursive_mutex> lock(UiMutex); slot = Ui.SelectedCharacterSlot(); present = Ui.SelectedCharacterPresent(); loginText = Ui.ActionState().LoginText; passwordText = Ui.ActionState().PasswordText; Ui.SetCharacterActionLocked(true); }
    if (!session || !session->Connected()) { CharacterActionInProgress.store(false); { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetCharacterActionLocked(false); } AddStatusLine("character: server session is not active"); return; }
    if (!present) { CharacterActionInProgress.store(false); { std::lock_guard<std::recursive_mutex> lock(UiMutex); Ui.SetCharacterActionLocked(false); } AddStatusLine("character: selected slot is empty"); return; }
    if (CharacterThread.joinable()) { CharacterThread.join(); }
    ...
    CharacterThread = std::thread([this, session, slot, wideLogin, widePassword]() mutable {
        ...
        std::optional<FLoginProbeResult> refresh;
        if (deleted.Ok) { session->Close(); refresh = RefreshCharacterSelectSession(wideLogin, widePassword, 2500); }
        { std::lock_guard<std::mutex> lock(CharacterMutex); PendingCharacterResult = std::move(deleted); PendingCharacterRefreshResult = std::move(refresh); PendingCharacterActionKind = 2; PendingCharacterSlot = slot; PendingCharacterName.clear(); PendingCharacterAppearance = {}; }
        CharacterActionInProgress.store(false);
    });
}
...
FStatus FClientFrontendRuntime::RunEventLoop() {
    ...
    while (Window.IsOpen() && Window.PumpMessages()) {
        ...
        std::string action;
        { std::lock_guard<std::recursive_mutex> lock(UiMutex); inputChanged = Ui.HandleInputFrame(input, client, Log); action = Ui.ConsumeLastAction(); }
        if (!action.empty()) { ProcessUiAction(action); inputChanged = true; }
        if (inputChanged) { RepaintDirty = true; }
        { std::lock_guard<std::mutex> lock(SessionMutex); Session.Tick(); }
        UpdateStageFromSession();
        PollLoginResult();
        PollCharacterResult();
        auto now = std::chrono::steady_clock::now();
        if (D3DInitialized.load() && (RepaintDirty || now - lastRender >= std::chrono::milliseconds(16))) { RenderFrame(); lastRender = now; RepaintDirty = false; }
...
void FClientFrontendRuntime::Shutdown() {
    ...
    if (CharacterThread.joinable()) { CharacterThread.join(); }
    ActiveServerSession.reset();
    { std::lock_guard<std::mutex> lock(SessionMutex); Session.Close(); }
    { std::lock_guard<std::mutex> renderLock(RenderMutex); RenderDevice.Shutdown(); }
    Window.Destroy();
    ShellCreated = false;
...
void FClientFrontendRuntime::UpdateStageFromSession() {
    ...
    { std::lock_guard<std::mutex> lock(SessionMutex); const auto& snapshot = Session.Snapshot(); stage = snapshot.Stage; stageText = snapshot.StageText; }
    if (stage == LastSessionStage && stage != EClientSessionStage::ProbeReceiving) { return; }
    LastSessionStage = stage;
    if (stage == EClientSessionStage::ProbeReceiving) { SetStage(stageText, 1.0f); }
    else if (stage == EClientSessionStage::Connected) { SetStage("connected; awaiting protocol", 1.0f); }
    else if (stage == EClientSessionStage::Failed) { SetStage("network probe failed", 1.0f); }
}
}
