...
void FPacketBuffer::Append(const uint8* data, size_t size) { Buffer.insert(Buffer.end(), data, data + size); }
...
FByteArray FPacketBuffer::MakeFrame(const FByteArray& payload) const {
    FByteArray frame;
    if (Mode == EPacketLengthMode::UInt16LE) { frame.push_back(uint8(payload.size() & 0xFF)); frame.push_back(uint8((payload.size() >> 8) & 0xFF)); }
    else { frame.push_back(uint8(payload.size() & 0xFF)); frame.push_back(uint8((payload.size() >> 8) & 0xFF)); frame.push_back(uint8((payload.size() >> 16) & 0xFF)); frame.push_back(uint8((payload.size() >> 24) & 0xFF)); }
