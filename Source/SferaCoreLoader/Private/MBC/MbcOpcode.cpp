...
namespace Sfera {
static std::string H(uint32 value) { std::ostringstream out; out << "0x" << std::uppercase << std::hex << value; return out.str(); }
...
const std::vector<FMbcOpcodeSpec>& MbcOpcodeTable() { static auto table = MakeOpcodes(); return table; }
const std::vector<FMbcBuiltinSpec>& MbcBuiltinTable() { static auto table = MakeBuiltins(); return table; }
const FMbcOpcodeSpec* FindMbcOpcode(uint8 opcode) { for (const auto& spec : MbcOpcodeTable()) { if (spec.Opcode == opcode) { return &spec; } } return nullptr; }
const FMbcBuiltinSpec* FindMbcBuiltin(uint8 subOpcode) { for (const auto& spec : MbcBuiltinTable()) { if (spec.SubOpcode == subOpcode) { return &spec; } } return nullptr; }
FMbcDecodedOpcode DecodeMbcOpcode(const FByteArray& code, uint32 off) {
    FMbcDecodedOpcode out;
    if (off >= code.size()) { out.Mnemonic = "eof"; out.Length = 0; out.Terminal = true; out.Known = false; return out; }
    uint8 opcode = code[off];
    if (opcode == 0x23) { out.Mnemonic = "end_program"; out.Length = 1; out.Terminal = true; out.Edges.push_back({"end_program", std::nullopt, "main-loop special: finish current program"}); return out; }
    if (opcode == 0x7C) { out.Mnemonic = "yield_program"; out.Length = 1; out.Terminal = true; out.Operands["resume_target"] = U(off + 1); out.Edges.push_back({"yield_resume", off + 1, "saved PC after yield; resumed by scheduler"}); return out; }
    const FMbcOpcodeSpec* spec = FindMbcOpcode(opcode);
    if (!spec) { out.Mnemonic = "unknown_" + H(opcode); out.Length = 1; out.Terminal = true; out.Known = false; out.Edges.push_back({"trap", std::nullopt, "opcode absent in recovered dispatcher table"}); return out; }
    out.Spec = spec;
    ...
    auto have = [&](uint32 n) { return off <= code.size() && n <= code.size() - off; };
    try {
        switch (spec->Format) {
        case EMbcOperandFormat::None: out.Length = 1; break;
        case EMbcOperandFormat::Trap: out.Length = 1; out.Terminal = true; out.Known = false; out.Edges.push_back({"trap", std::nullopt, spec->Semantic}); break;
        case EMbcOperandFormat::U8: if (!have(2)) { throw std::runtime_error("truncated u8"); } out.Length = 2; out.Operands["value"] = U(code[off + 1]); break;
        case EMbcOperandFormat::U16: if (!have(3)) { throw std::runtime_error("truncated u16"); } out.Length = 3; out.Operands["value"] = U(Mbc::ReadU16(code, off + 1)); break;
        case EMbcOperandFormat::ProgramI16: if (!have(3)) { throw std::runtime_error("truncated program_i16"); } out.Length = 3; out.Operands["program_index"] = S(Mbc::ReadI16(code, off + 1)); break;
        case EMbcOperandFormat::Rel16: { if (!have(3)) { throw std::runtime_error("truncated rel16"); } out.Length = 3; int32 rel = Mbc::ReadI16(code, off + 1); uint32 target = static_cast<uint32>(int32(off + 1) + rel); out.Operands["rel"] = S(rel); out.Operands["target"] = U(target); out.Edges.push_back({"jmp", target, ""}); out.Terminal = true; break; }
        case EMbcOperandFormat::Rel32: { if (!have(5)) { throw std::runtime_error("truncated rel32"); } out.Length = 5; int32 rel = Mbc::ReadI32(code, off + 1); uint32 target = static_cast<uint32>(int32(off + 1) + rel); out.Operands["rel"] = S(rel); out.Operands["target"] = U(target); out.Edges.push_back({"jmp", target, ""}); out.Terminal = true; break; }
        case EMbcOperandFormat::JFalseRel16: { if (!have(3)) { throw std::runtime_error("truncated jfalse_rel16"); } out.Length = 3; int32 rel = Mbc::ReadI16(code, off + 1); uint32 target = static_cast<uint32>(int32(off + 1) + rel); out.Operands["rel"] = S(rel); out.Operands["target"] = U(target); out.Operands["fallthrough"] = U(off + out.Length); out.Edges.push_back({"jfalse", target, "condition false/zero"}); out.Edges.push_back({"jtrue_fallthrough", off + out.Length, "condition true/non-zero"}); break; }
        case EMbcOperandFormat::JFalseRel32: { if (!have(5)) { throw std::runtime_error("truncated jfalse_rel32"); } out.Length = 5; int32 rel = Mbc::ReadI32(code, off + 1); uint32 target = static_cast<uint32>(int32(off + 1) + rel); out.Operands["rel"] = S(rel); out.Operands["target"] = U(target); out.Operands["fallthrough"] = U(off + out.Length); out.Edges.push_back({"jfalse", target, "condition false/zero"}); out.Edges.push_back({"jtrue_fallthrough", off + out.Length, "condition true/non-zero"}); break; }
        case EMbcOperandFormat::LogicalOrRel16: { if (!have(3)) { throw std::runtime_error("truncated or_rel16"); } out.Length = 3; int32 rel = Mbc::ReadI16(code, off + 1); uint32 target = static_cast<uint32>(int32(off + 1) + rel); out.Operands["target"] = U(target); out.Edges.push_back({"jtrue", target, "short-circuit OR"}); out.Edges.push_back({"jfalse_fallthrough", off + out.Length, "false falls through"}); break; }
        case EMbcOperandFormat::LogicalAndRel16: { if (!have(3)) { throw std::runtime_error("truncated and_rel16"); } out.Length = 3; int32 rel = Mbc::ReadI16(code, off + 1); uint32 target = static_cast<uint32>(int32(off + 1) + rel); out.Operands["target"] = U(target); out.Edges.push_back({"jfalse", target, "short-circuit AND"}); out.Edges.push_back({"jtrue_fallthrough", off + out.Length, "true falls through"}); break; }
        case EMbcOperandFormat::CallRel32: { if (!have(5)) { throw std::runtime_error("truncated call_rel32"); } out.Length = 5; int32 rel = Mbc::ReadI32(code, off + 1); uint32 target = static_cast<uint32>(int32(off + 1) + rel); out.Operands["target"] = U(target); out.Operands["return"] = U(off + out.Length); out.Edges.push_back({"call_rel32", target, ""}); out.Edges.push_back({"call_return", off + out.Length, ""}); break; }
        case EMbcOperandFormat::DataRef: if (!have(6)) { throw std::runtime_error("truncated data_ref"); } out.Length = 6; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["data_offset"] = U(Mbc::ReadU32(code, off + 2)); break;
        case EMbcOperandFormat::TypedImm32: if (!have(6)) { throw std::runtime_error("truncated typed_imm32"); } out.Length = 6; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["value_u32"] = U(Mbc::ReadU32(code, off + 2)); out.Operands["value_i32"] = S(Mbc::ReadI32(code, off + 2)); if (code[off + 1] == Mbc::TypeFloat) { out.Operands["value_float"] = std::to_string(Mbc::FloatFromU32(Mbc::ReadU32(code, off + 2))); } break;
        case EMbcOperandFormat::TypedImmU16: if (!have(4)) { throw std::runtime_error("truncated typed_imm_u16"); } out.Length = 4; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["value"] = U(Mbc::ReadU16(code, off + 2)); break;
        case EMbcOperandFormat::TypedImmI8: if (!have(3)) { throw std::runtime_error("truncated typed_imm_i8"); } out.Length = 3; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["value"] = S(Mbc::Sign8(code[off + 2])); break;
        case EMbcOperandFormat::ImportStubU32: if (!have(5)) { throw std::runtime_error("truncated import_stub"); } out.Length = 5; out.Terminal = true; out.Operands["payload_u32"] = U(Mbc::ReadU32(code, off + 1)); out.Edges.push_back({"import_stub", std::nullopt, spec->Semantic}); break;
        case EMbcOperandFormat::InlineSpan: if (!have(7)) { throw std::runtime_error("truncated inline_span"); } out.Length = 7; out.Operands["data_offset"] = U(Mbc::ReadU32(code, off + 1)); out.Operands["length"] = U(Mbc::ReadU16(code, off + 5)); break;
        case EMbcOperandFormat::TypedSpanRef: case EMbcOperandFormat::TypedSpanInline: if (!have(10)) { throw std::runtime_error("truncated typed_span"); } out.Length = 10; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["data_offset"] = U(Mbc::ReadU32(code, off + 2)); out.Operands["length"] = U(Mbc::ReadU32(code, off + 6)); break;
        case EMbcOperandFormat::ArrayAbs: if (!have(16)) { throw std::runtime_error("truncated array_abs"); } out.Length = 16; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["element_size"] = U(Mbc::ReadU16(code, off + 2)); out.Operands["base"] = U(Mbc::ReadU32(code, off + 4)); out.Operands["span"] = U(Mbc::ReadU32(code, off + 8)); out.Operands["count"] = S(Mbc::ReadI32(code, off + 12)); break;
        case EMbcOperandFormat::Array2Checked: if (!have(8)) { throw std::runtime_error("truncated array2_checked"); } out.Length = 8; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["element_size"] = U(Mbc::ReadU16(code, off + 2)); out.Operands["count"] = S(Mbc::ReadI32(code, off + 4)); break;
        case EMbcOperandFormat::Array2: if (!have(4)) { throw std::runtime_error("truncated array2"); } out.Length = 4; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["element_size"] = U(Mbc::ReadU16(code, off + 2)); break;
        case EMbcOperandFormat::SliceOffsetRef: if (!have(4)) { throw std::runtime_error("truncated slice_offset_ref"); } out.Length = code[off + 1] == Mbc::TypeSlice ? 8 : 4; if (!have(out.Length)) { throw std::runtime_error("truncated slice_offset_ref_ext"); } out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["offset"] = U(Mbc::ReadU16(code, off + 2)); if (out.Length == 8) { out.Operands["length"] = U(Mbc::ReadU32(code, off + 4)); } break;
        case EMbcOperandFormat::SliceOffsetSpan: if (!have(8)) { throw std::runtime_error("truncated slice_offset_span"); } out.Length = 8; out.Operands["type"] = U(code[off + 1]); out.Operands["type_name"] = Mbc::TypeName(code[off + 1]); out.Operands["offset"] = U(Mbc::ReadU16(code, off + 2)); out.Operands["length"] = U(Mbc::ReadU32(code, off + 4)); break;
        case EMbcOperandFormat::Prologue: { if (!have(2)) { throw std::runtime_error("truncated prologue count"); } int8 signedCount = Mbc::Sign8(code[off + 1]); uint32 count = signedCount < 0 ? uint32(-signedCount) : uint32(signedCount); out.Length = 2 + count * 5; if (!have(out.Length)) { throw std::runtime_error("truncated prologue descriptors"); } out.Operands["signed_count"] = S(signedCount); out.Operands["descriptor_count"] = U(count); for (uint32 i = 0; i < count; ++i) { uint32 p = off + 2 + i * 5; out.Operands["arg" + U(i) + "_type"] = U(code[p]); out.Operands["arg" + U(i) + "_data_offset"] = U(Mbc::ReadU32(code, p + 1)); } break; }
        case EMbcOperandFormat::Builtin: { if (!have(2)) { throw std::runtime_error("truncated builtin"); } out.Length = 2; uint8 sub = code[off + 1]; const FMbcBuiltinSpec* builtin = FindMbcBuiltin(sub); out.Builtin = builtin; out.Operands["subopcode"] = U(sub); if (builtin) { out.Mnemonic = builtin->Mnemonic; out.Operands["target_ea"] = H(builtin->TargetEa); out.Operands["target_name"] = builtin->TargetName; out.Operands["builtin_semantic"] = builtin->Semantic; out.Operands["builtin_confidence"] = builtin->Confidence; } else { out.Mnemonic = "unknown_builtin_" + H(sub); out.Known = false; } break; }
        }
    } catch (const std::exception& e) { out.Length = 1; out.Known = false; out.Terminal = false; out.Operands["decode_error"] = e.what(); out.Mnemonic += "_truncated"; out.Edges.clear(); }
    if (opcode == 0x72 || opcode == 0x74) { out.Terminal = true; out.Edges.push_back({"return", std::nullopt, spec->Semantic}); }
    if (opcode == 0x48) { out.Terminal = true; out.Edges.push_back({"halt", std::nullopt, spec->Semantic}); }
    return out;
}
}
