...
namespace Sfera {
void FSpatialIndex::Clear() { Items.clear(); Buckets.clear(); }
...
std::vector<int64> FSpatialIndex::CoveredCells(FBox2 bounds) const {
    ...
    for (int32 y = minY; y <= maxY; ++y) { for (int32 x = minX; x <= maxX; ++x) { out.push_back(MakeKey(x, y)); } }
    return out;
}
void FSpatialIndex::Insert(uint32 id, FBox2 bounds, std::string tag) {
    ...
    for (int64 key : item.Cells) { Buckets[key].push_back(id); }
...
void FSpatialIndex::Remove(uint32 id) {
    ...
    for (int64 key : it->second.Cells) {
        ...
        if (ids.empty()) { Buckets.erase(bucket); }
    }
    Items.erase(it);
}
std::vector<uint32> FSpatialIndex::Query(FBox2 area) const {
    ...
    for (int64 key : CoveredCells(area)) {
        ...
        for (uint32 id : bucket->second) {
            ...
            auto it = Items.find(id);
            if (it != Items.end() && it->second.Bounds.Intersects(area)) { result.push_back(id); }
        }
    }
    return result;
}
std::optional<FSpatialItem> FSpatialIndex::Find(uint32 id) const { auto it = Items.find(id); if (it == Items.end()) { return std::nullopt; } return it->second; }
}
