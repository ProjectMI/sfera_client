...
FStatus FMbcModule::ParseImage()
{
    ...
    {
        HeaderProbe.Commentary = ex.what();
        return FStatus::Error(EStatusCode::InvalidData, HeaderProbe.Commentary);
    }
}
...
const FMbcProgram* FMbcModule::ProgramByName(std::string_view name) const
{
    for (const auto& p : ProgramTable)
    {
        if (p.Name == name)
        {
            return &p;
...
const FMbcProgram* FMbcModule::ProgramForOffset(uint32 codeOffset) const
{
    for (const auto& p : ProgramTable)
    {
        if (p.Contains(codeOffset))
        {
            return &p;
        }
    }
...
const FMbcFunction* FMbcModule::FunctionByName(std::string_view name) const
{
...
const FMbcFunction* FMbcModule::FunctionAtOffset(uint32 codeOffset) const
{
    for (const auto& f : FunctionTable)
    {
        if (f.CodeOffset == codeOffset)
        {
            return &f;
        }
    }
