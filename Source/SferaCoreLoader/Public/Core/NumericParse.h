...
inline std::string_view TrimView(std::string_view text) {
    while (!text.empty() && static_cast<unsigned char>(text.front()) <= ' ') { text.remove_prefix(1); }
    while (!text.empty() && static_cast<unsigned char>(text.back()) <= ' ') { text.remove_suffix(1); }
    return text;
}

inline bool IsFullyConsumed(const char* end, const char* stop) {
    while (end < stop && static_cast<unsigned char>(*end) <= ' ') { ++end; }
    return end == stop;
}
