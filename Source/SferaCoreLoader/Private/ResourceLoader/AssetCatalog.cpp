...
void FAssetCatalog::BuildFromFileSystem(const FFileSystem& fileSystem) {
    ...
    Lookup.clear();
    for (size_t i = 0; i < Records.size(); ++i) { Lookup[PathUtils::NormalizeForLookup(Records[i].RelativePath)] = i; }
}

std::vector<FFileRecord> FAssetCatalog::FindByKind(EResourceKind kind) const {
    std::vector<FFileRecord> result;
    for (const auto& record : Records) { if (GuessResourceKind(record.RelativePath) == kind) { result.push_back(record); } }
    return result;
}
