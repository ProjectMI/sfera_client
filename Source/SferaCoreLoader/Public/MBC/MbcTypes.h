...
{ 
	std::string Magic; 
	uint32 ChecksumOrTag = 0; 
	uint32 ModuleTag = 0;
	uint32 CodeSize = 0; 
	uint32 DataSize = 0; 
	uint32 CodeFileOffset = Mbc::CodeFileOffset;
};
...
struct FMbcProgram 
{
	...
	uint8 QueueId = 0;
	uint32 Unknown48 = 0;
	int8 State() const { return Mbc::Sign8(StateRaw); } 
	uint32 FileStart() const { return Mbc::CodeFileOffset + Start; } 
	uint32 FileEnd() const { return Mbc::CodeFileOffset + End; } 
	bool Contains(uint32 codeOffset) const { return Start <= codeOffset && codeOffset <= End; } 
};
...
{ 
	...
	std::string Name; 
	uint32 CodeOffset = 0;
	uint32 ProgramIndexRaw = 0; 
	uint32 FlagsOrModule = 0; 
	int32 ProgramIndex() const { return static_cast<int32>(ProgramIndexRaw); }
	bool IsImport() const { return ProgramIndex() < 0; } 
	uint32 FileOffset() const { return Mbc::CodeFileOffset + CodeOffset; } 
};

struct FMbcArrayGuard 
{ 
	uint32 Index = 0; 
	uint32 BeginGuardOffset = 0;
	uint32 EndGuardOffset = 0; 
};
...
struct FMbcValue 
{ 
	uint8 Type = Mbc::TypeInt;
	int32 IntValue = 0; 
	float FloatValue = 0.0f; 
	FMbcSlice Slice;
	uint32 DataOffset = 0;
	bool IsLValue = false; 
	static FMbcValue Int(int32 value); 
	static FMbcValue Float(float value);
	static FMbcValue Char(uint8 value);
	static FMbcValue SliceValue(uint32 offset, uint32 length, uint32 capacity); 
	static FMbcValue Ref(uint8 type, uint32 dataOffset); 
};

struct FMbcRuntimeEvent 
{ 
	std::string Kind;
	std::string Name;
